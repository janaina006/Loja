...
if(isset($_POST['cli_email'])){
    ...
    if($cliente->GetClienteEmail($cliente->getCli_email()) > 0){
        ...
        $msg = "Olá cliente, uma nova senha foi solicitada por você,
        acesse o site: " . Config::SITE_NOME;
        $msg .= "<br> Nova Senha = " . $novasenha;
        $destinatarios = array($cliente->getCli_email(), Config::SITE_EMAIL_ADM);
        $email->Enviar($assunto, $msg, $destinatarios);

        echo '<div class="alert alert-success"> <p> Olá, foi enviada
        uma nova senha pra acesso ao site em seu email de cadastro!!</p></div>';
        Rotas::Redirecionar(5, Rotas::pag_ClienteLogin());


    }else{
        echo '<div class="alert alert-danger"> <p>Este email
        não está cadastrado na loja!</p></div>';
